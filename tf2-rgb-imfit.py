...
def setup_inputs(opts):
    ...
    target_tensor = tf.keras.Input(shape=input_image.shape,
                                 name='target')

    max_row = tf.keras.Input(shape=(),
                           dtype=tf.int32,
                           name='max_row')
...
def full_optimize(opts, inputs, models, state, sess,
    ...
    # Convert state params to tensor
    params_tensor = tf.convert_to_tensor(state.params[None,:], dtype=tf.float32)
    models.full.params.assign(params_tensor)
    ...
    for i in range(opts.full_iter):
        ...
        # Get next batch
        input_batch, max_row = next(iter(dataset))

        # Gradient tape
        with tf.GradientTape() as tape:
            # Forward pass
            gabor = models.full.gabor
        ...
        # Get gradients
        grads = tape.gradient(loss, models.full.trainable_variables)
        
        # Apply gradients
        optimizer.apply_gradients(zip(grads, models.full.trainable_variables))
...
def randomize(params, rstdev, ncopy=None):
    ...
    bump = tf.random.normal(pshape, stddev=rstdev)
...
def local_optimize(opts, inputs, models, state,
    ...
    new_gabor = results['gabor'][fidx]  # Shape: [H, W, C, models, 1, 1]
    ...
    if do_update:
        ...
        # Properly reshape new_gabor to match state.gabor's shape [H,W,C]
        # Remove the last two singleton dimensions and take first model
        reshaped_gabor = new_gabor[..., 0, 0, 0]  
