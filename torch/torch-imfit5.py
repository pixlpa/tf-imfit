...
class ImageFitter:
    ...
    def load_weights(self,path):
        ...
        with torch.no_grad():
            ...
            self.model.psi.data = torch.from_numpy(np.array([weights[6],weights[7],weights[8]])).to(device)
            self.model.amplitude.data = torch.from_numpy(np.array([weights[9],weights[10],weights[11]])).to(device)
