...
class GaborModel(tf.keras.Model):
    ...
    def compute_gabor(self):
        ...
        u,v,r,p,l,t,s,h = [cparams[:,None,None,:,i] 
    ...
    def compute_loss(self, gabor):
        approx = tf.reduce_sum(gabor, axis=3)
        ...
        constraints = tf.stack(pairwise_constraints, axis=2)
        con_sqr = tf.minimum(constraints, 0)**2
        con_losses = tf.reduce_sum(con_sqr, axis=2)
        con_loss_per_fit = tf.reduce_sum(con_losses, axis=1)
        
        # Compute approximation error
        if self.target is not None:
            err = tf.multiply((self.target - approx), self.weight)
            err_sqr = 0.5*err**2
            err_loss_per_fit = tf.reduce_mean(err_sqr, axis=(1,2))
            
            loss_per_fit = con_loss_per_fit + err_loss_per_fit
            total_loss = tf.reduce_mean(loss_per_fit)
        else:
            total_loss = tf.reduce_mean(con_loss_per_fit)
            
        return total_loss, approx, gabor, con_losses

    @tf.function
    def train_step(self):
        with tf.GradientTape() as tape:
            gabor = self.compute_gabor()
            loss, approx, _, con_losses = self.compute_loss(gabor)
            
        if self.target is not None:
            grads = tape.gradient(loss, [self.params])
            self.optimizer.apply_gradients(zip(grads, [self.params]))
            
        return loss, approx, gabor, con_losses

    def call(self, inputs):
        gabor = self.compute_gabor()
        _, approx, _, _ = self.compute_loss(gabor)
        return approx
...
def snapshot(cur_gabor, cur_approx,
    ...
    if not opts.preview_size:
        ...
        models.preview.max_row = max_rowval
        preview_image = models.preview(None)[0]
